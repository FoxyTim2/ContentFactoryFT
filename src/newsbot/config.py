...
def load_settings() -> Settings:
    ...
    start_mode = os.getenv("START_MODE", "now").strip().lower()
    if start_mode not in {"now", "lookback"}:
        raise ValueError("START_MODE must be 'now' or 'lookback'")

    russia_policy = os.getenv("RUSSIA_POLICY", "review").strip().lower()
    if russia_policy not in {"review", "drop"}:
        raise ValueError("RUSSIA_POLICY must be 'review' or 'drop'")
    ...
    return Settings(
        ...
        admin_review_chat_id=os.getenv("ADMIN_REVIEW_CHAT_ID") or None,
        russia_policy=russia_policy,
        max_output_chars=int(os.getenv("MAX_OUTPUT_CHARS", "1200")),
    )
